...
namespace RomForge.Core.IO
{
    ...
    {
        public static string? ResolveIm1Path(
            string imgsBasePath,
            DatHeader header,
            int imageNumber
        ) => imageNumber > 0 ? Resolve(imgsBasePath, header, imageNumber, "a") : null;

        public static string? ResolveIm2Path(
            string imgsBasePath,
            DatHeader header,
            int imageNumber
        ...
        )
        {
            var path = Path.Combine(
                imgsBasePath,
                BuildRelativeLocalPath(header, imageNumber, suffix)
            );
        ...
        /// </summary>
        internal static string BuildRelativeLocalPath(
            DatHeader header,
            int imageNumber,
            string suffix
        )
