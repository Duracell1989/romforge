...
internal sealed class BatchProgressRunner
{
    ...
    public async Task<int> RunAsync<T>(BatchProgressOperation<T> operation)
    {
        ...
        ProgressWindowVM progress = new ProgressWindowVM(
            operation.Targets.Count,
            operation.IsCancellable
        );
        Task<List<string>> operationTask = RunCoreAsync(operation, progress);
        await _notifier.ShowProgressAsync(operation.Title, progress, operationTask);

        List<string> errors = await operationTask;
        int succeeded = operation.Targets.Count - errors.Count;
        _logger.Information(
    ...
    )
    {
        ...
        try
        {
            for (int i = 0; i < operation.Targets.Count; i++)
            {
                if (progress.CancellationToken.IsCancellationRequested)
                    break;

                T item = operation.Targets[i];
                ...
                string? error = await operation.ProcessAsync(item, progress);
